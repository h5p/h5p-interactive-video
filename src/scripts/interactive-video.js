...
function InteractiveVideo(params, id, contentData) {
    ...
    self.initialize = function() {
        ...
        }, self.contentId, undefined, undefined, {
            parent: self
        });
...
InteractiveVideo.prototype.setCaptionTracks = function(tracks) {
    ...
    /** NSTYLIANIDES
     * MARK: A
     * When ```self.controls.$volume``` is not rendered because the device is a mobile/tablet, the following line fails to append 
     * control button on the DOM therefore the MARK: B line will fail.
     * So we just create the ```self.controls.$volume``` element and keep it hidden in MARK: C
     * */
    ...
    /** NSTYLIANIDES
     * MARK: B
     * This line fails in mobile/tablet because ```self.captionsTrackSelector.control``` has not appended in the DOM in the previous call (MARK: A). 
     */
 ...
 */
InteractiveVideo.prototype.toggleBookmarksChooser = function(show, params = {
    keepStopped: false,
    firstPlay: false
}) {
    if (this.controls.$bookmarksButton) {
        ...
        this.controls.$bookmarksChooser
            .css({
                maxHeight: show ? this.controlsCss.maxHeight : '32px'
            })
 ...
 * @param {boolean} [params.firstPlay] If first time.
 */
InteractiveVideo.prototype.toggleEndscreensChooser = function(show, params = {
    keepStopped: false,
    firstPlay: false
}) {
...
InteractiveVideo.prototype.addEndscreen = function(id, tenth) {
    ...
    self.trigger('endscreenAdded', {
        'endscreen': $endscreenMarker
    });
...
InteractiveVideo.prototype.attachControls = function($wrapper) {
    ...
    /** NSTYLIANIDES
     * MARK: C
     * Create the button anyway and keep it hidden by default (h5p-hide). If is not Android or IOS then remove the h5p-hide class
     * */
    self.controls.$volume = self.createButton('mute', 'h5p-control h5p-hide', $right, self.toggleMute);

    if (!isAndroid() && !isIpad()) {

        /**
         * REMOVE the h5p-hide class
         */
        self.controls.$volume.removeClass("h5p-hide");
    ...
    self.controls.$buffered = $('<div/>', {
        'class': 'h5p-buffered',
        prependTo: self.controls.$slider
    });
};
...
InteractiveVideo.prototype.getCopyrights = function() {
    ...
    if (self.hasMainSummary()) {
        ...
        if (instance !== undefined) {
            ...
            summaryCopyrights.addContent(H5P.getCopyrights(instance, {
                action: self.options.summary.task
            }, self.contentId));
